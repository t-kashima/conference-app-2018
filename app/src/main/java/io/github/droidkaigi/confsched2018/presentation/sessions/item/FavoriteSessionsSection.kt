...
class FavoriteSessionsSection(private val fragment: Fragment) : Section() {
    fun updateSessions(
            sessions: List<Session>,
            onFavoriteClickListener: (Session.SpeechSession) -> Unit
    ) {
        val sessionItems = sessions.map {
            FavoriteSessionItem(it as Session.SpeechSession, onFavoriteClickListener, fragment) as SessionItem
        }
